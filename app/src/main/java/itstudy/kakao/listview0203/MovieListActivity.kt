...
class MovieListActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        /*
        movieAdapter = ArrayAdapter<Movie>(
            this,
        ...
        listView?.setOnScrollListener(
            object: AbsListView.OnScrollListener{
                ...
                override fun onScroll(
                    ...
                    //하단에서 스크롤 했는지 여부를 판단
                    //첫번째 보여지는 데이터 와 보여지는 데이터 개수를 더한 것이
                    //전체 데이터 개수보다 크거나 같다면 가장 하단으로 내려온 것임
                    lastItemVisibleFlag = totalItemCount > 0 &&
                            firstVisibleItem + visibleItemCount >= totalItemCount
                }
                //스크롤의 상태가 변경될 때 호출되는 메소드
                override fun onScrollStateChanged(view: AbsListView?, scrollState: Int) {
                    //스크롤이 멈추가 가장 하단에서 스크롤 한 것이라면 데이터 업데이트
                    if(scrollState == AbsListView.OnScrollListener.SCROLL_STATE_IDLE
                        ...
                        if(pageno * cnt >= count!!){
                            ...
                            "더이상 데이터가 없음", Toast.LENGTH_LONG).show()
